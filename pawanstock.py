...
import websocket  # Added to fix the SmartApi connection error
...
# Setup logzero file path
log_file = os.path.join(LOG_DIR, "algo_log.txt")
...
if "obj" not in st.session_state:
    try:
        ...
        logger.info(f"API Session Generated for Client: {CLIENT_ID}")
...
def place_live_order(symbol, token, side, ltp):
    try:
        ...
        logger.info(f"ORDER PLACED: {symbol} | Side: {side} | Price: {ltp} | ID: {order_id}")
        return order_id
    except Exception as e:
...
if page == "🚨 PANIC BUTTON":
    if st.button("🚨 EXIT ALL & STOP SYSTEM"):
        ...
        logger.critical("PANIC BUTTON TRIGGERED - SYSTEM HALTED")
